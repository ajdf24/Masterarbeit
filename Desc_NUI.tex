...
Im leichte Gegenstände zum platzieren werden diese Gegenstände von Menschen zwischen Daumen und Zeigefinger - größere oder schwerere Gegenstände auch schon mal unter Zuhilfenahme des Mittelfingers - gegriffen und auf der gewünschten Stelle platziert. Siehe Abbildung \ref{chess_hand}.
...
Um Objekte auf einer Fläche zu drehen, nutzen Menschen in der Regel den Zeigefinger, um das Objekt auf einer Seite anzustoßen. Dieser Impuls dreht dann das entsprechende Objekt. Für das \ac{NUI} bedeutet das also, das ein ausgestreckter Zeigefinger der in der Luft rotiert das Objekt rotieren soll. In Abbildung \ref{rotate_1} ist diese Geste einmal schematisch dargestellt.
...
Im Umkehrschluss bedeutet dies, das auch diese Geste bekannt sein muss. Das \ac{NUI} muss entsprechend zwei drehende Finger auch als Kommando zum Drehen auffassen. In Abbildung \ref{rotate_2} ist diese Geste dargestellt. 
...
Die ersten drei Befehle sind eindeutig. Es ist klar, das der selektierte Objekt gesetzt, entfernt oder gedreht werden soll. Ist nichts Selektiert, kann der Befehl nicht ausgeführt werden. Andernfalls hat die Eingabe des Sprachkommandos eine unmittelbare Folge.

Das ändern des Materials hingegen ist komplexer. Der Befehl hat eine Variable, nämlich die Art des Materials. Das Material muss im Programm bekannt sein, um es auswählen zu können. Somit ist der Satz "`Ändere das Material zu Stein."' ein eindeutiger Befehl. Das Interface muss jedoch so Programmiert werden, dass der Auslöser "`Ändere das Material"' ist. "`Stein"' muss zusätzlich erkannt werden, diese Variable enthält die eigentliche Information. 

Ist der Auslöser richtig erkannt wurden, so muss das Interface das Entsprechende Material wählen. Ist das Material dem Programm nicht bekannt, so muss es dies an den Nutzer kommunizieren. 
...
"`Platzieren"', "`postieren"' und "`hintun"' hingegen sind eindeutige alternativen für das angedachte \ac{NUI}. Es ist durchaus aus im Bereich des möglichen, das ein Nutzer "`Block platzieren"' oder "`Block postieren"' sagt an Stelle von "`Block setzen"'. Das Interface muss also bei den Befehlen:
...
Eine ist die so genannte \ac{SRGS} des \ac{W3C}, welche Sprachen grammatikalisch beschreiben. Hier können in \ac{XML} konformer weise, Wortschemen eingegeben werden, um einer Maschine zu verstehen zu geben, wie ein Mensch, ein Befehl aussprechen würde. 
...
\begin{minipage}{5.5cm}
\vspace{0.5cm}
 Das Drehen der Finger ist nicht ganz Selbstbechreibend, jedoch gleicht das adaptive \ac{NUI}, welche die Geste interpretierert wieder aus, so das eintsprechend eine Drehbewegung erkannt werden kann.
\end{minipage}
\\\\
\begin{minipage}{4cm}
\vspace{0.5cm}
...
\begin{minipage}{5.5cm}
\vspace{0.5cm}
 Die Geste Erwartungskonform zur realität gehalten und somit Erwartungskonform in jeder Hinsicht, da keine andere Handlung ausgeführt wird.
\end{minipage}\hspace{0.5cm}
\begin{minipage}{5.5cm}
\vspace{0.5cm}
...
\begin{minipage}{5.5cm}
\vspace{0.5cm}
 Durch das \ac{NUI} wird eine Ungenauigkeit der Nutzer abgefangen. Jedoch ist die Geste so eindeutig, das sie nicht weiter Individualisierbar ist.
\end{minipage}\hspace{0.5cm}
\begin{minipage}{5.5cm}
\vspace{0.5cm}
...
Sprache ist zwar nicht immer eindeutig, jedoch wird sie dies im Kontext betrachtet. Befehle, welche also zu einem \ac{NUI} gesprochen werden, werden entweder Verstanden und ausgeführt oder nicht. Weshalb eine überprüft anhand der ISO 9241 Teil 110 nicht Sinnvoll ist.
